...
final class CallEnqueueOnSubscribe<T> implements Observable.OnSubscribe<Response<T>> {
    ...
    public void call(Subscriber<? super Response<T>> subscriber) {
        ...
        final CallArbiter callArbiter = new CallArbiter(mo8clone, subscriber);
        subscriber.add(callArbiter);
        subscriber.setProducer(callArbiter);
        mo8clone.enqueue(new Callback<T>() { // from class: retrofit2.adapter.rxjava.CallEnqueueOnSubscribe.1
